...
for file_i in range(len(files)):
	...
	if len(depth_index_AABW) == 0:
		#Assume below 4000m depth the AABW
	...
	transport_AAIW	= np.sum(ma.masked_where(water_prop != 2.0, vel_baroclinic * layer_field * grid_x), axis = 1)
	transport_NADW	= np.sum(ma.masked_where(water_prop != 3.0, vel_baroclinic * layer_field * grid_x), axis = 1)
	transport_AABW	= np.sum(ma.masked_where(water_prop != 4.0, vel_baroclinic * layer_field * grid_x), axis = 1)
