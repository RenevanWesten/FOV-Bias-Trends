...
def ReadinDataFOV(filename):
	...
	FW		    = fh.variables['FW_transport'][:]	#Freshwater (Sv)

	fh.close()
    
    #Add a minus sign to indicate the transport into the Indian Ocean
	return time, -FW
...
time_control, FOV_control	            = ReadinDataFOV(directory+'Ocean/ITF_PI_control_LR.nc')
time_control_month, salt_control_month	= ReadinDataFOV(directory+'Ocean/ITF_PI_control_LR_monthly.nc')
time_rcp, FOV_rcp		                = ReadinDataFOV(directory+'Ocean/ITF_RCP_LR.nc')
time_control			                += 1599
...
fh = netcdf.Dataset(directory+'Ocean/SALT_SO_'+str(depth_min)+'_'+str(depth_max)+'m_trend_year_21-100_PI_control_LR.nc', 'r')
...
files = glob.glob(directory+'Data/PI_control_LR/ITF/iHESP_data_year_0050.nc')
files.sort()
...
vel_all		= ma.masked_all((len(files), len(depth)))
...
for file_i in range(len(files)):
	...
	vel_all[file_i]	   = np.sum(u_vel * grid_y_u_norm, axis = 1) * 100.0
...
ax.plot([1600, 1600, 1610, 1610, 1600], [0.0, 0.45, 0.45, 0.0, 0.0], '--k', linewidth = 1.0)
ax.plot([1610, 1765], [0.45, -0.051], ':k', linewidth = 0.5, zorder = 0)
ax.plot([1610, 1765], [0.0, -0.15], ':k', linewidth = 0.5, zorder = 0)
ax.plot([1600, 1635], [0.45, -0.051], ':k', linewidth = 0.5, zorder = 0)
ax.plot([1600, 1635], [0.0, -0.15], ':k', linewidth = 0.5, zorder = 0)

ax.set_title('b) Freshwater transport (into the Indian Ocean) at 115$^{\circ}$E, LR-CESM')
...
ax3 	= fig.add_axes([0.18, 0.30, 0.20, 0.13])

ax3.plot(time_control_month, salt_control_month, '-k', linewidth = 1.5)
ax3.set_xlim(1, 11)
...
ax.set_title('d) Zonal velocity, LR-CESM, PI control (model year 50)')
...
ax.set_title('f) Salinity, LR-CESM, PI control (model year 50)')
