...
files = glob.glob(directory+'Data/PI_control_LR/'+section_name+'/iHESP_data_year_*.nc')
files.sort()
...
depth_index_ABW	= np.where((depth >= 3000) & (transport_clin >= 0))[0][0]
...
lon_AIW_index		= np.where(age_all[depth_index_AIW].mask == False)[0]
lon_NADW_index		= np.where(age_all[depth_index_NADW].mask == False)[0]
lon_ABW_index		= np.where(age_all[depth_index_ABW].mask == False)[0]
...
if year_plot == 50:
    ax.set_title('b) Age, LR-CESM, PI control (model year 50)')
    
if year_plot == 450:
    ax.set_title('d) Age, LR-CESM, PI control (model year 450)')
