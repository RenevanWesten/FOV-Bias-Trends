...
time_control, FW_control	= ReadinDataFW(directory+'Ocean/ITF_PI_control_HR.nc')
time_rcp, FW_rcp		    = ReadinDataFW(directory+'Ocean/ITF_RCP_HR.nc')
time_control			    += 1599

print('Trend model year:', int(time_rcp[150]),'-', int(time_rcp[250]))
print()
trend, error, p_value = SignificantTrend(time_rcp[150:251], FW_rcp[150:251])
...
graph_control	= ax.plot(time_control, FW_control, '-k', linewidth = 1.5, label = 'PI control')
graph_rcp	= ax.plot(time_rcp, FW_rcp, '-r', linewidth = 1.5, label = 'Hist/RCP8.5')
...
ax.set_title('a) Freshwater transport (into the Indian Ocean) at 115$^{\circ}$E, HR-CESM')
