...
time_control, transport_control	= ReadinData(directory+'Ocean/AMOC_transport_depth_'+str(depth_min)+'-'+str(depth_max)+'m_PI_control_LR.nc')
time_rcp, transport_rcp		    = ReadinData(directory+'Ocean/AMOC_transport_depth_'+str(depth_min)+'-'+str(depth_max)+'m_RCP_LR.nc')
time_control			       += 1599
#-----------------------------------------------------------------------------------------

print('Trend model year:', int(time_control[129]-1599),'-', int(time_control[-2]-1599))
print()
trend, error, p_value = SignificantTrend(time_control[129:-1], transport_control[129:-1])
...
fh = netcdf.Dataset(directory+'Ocean/SST_NA_trend_year_21-100_PI_control_LR.nc', 'r')
...
ax.set_title('b) AMOC strength at 26$^{\circ}$N, LR-CESM')
